...
export function buildBlob(hue: number): THREE.Group {
  ...
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.23, 0.42, 8, 18), mat);
  torso.position.y = 0.9; // spans ~0.46..1.34, crotch high enough for real thighs
  torso.scale.set(1, 1, 0.85);
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.18, 20, 16), mat);
  head.position.y = 1.42; // slight neck indent against the shoulder curve
  body.add(torso, head);

  const legL = limb(0.115, 0.35, -0.105, 0.56, -0.05); // legs: proper thighs, near-touching
  const legR = limb(0.115, 0.35, 0.105, 0.56, 0.05);
  const armL = limb(0.09, 0.38, -0.16, 1.12, -0.1); // arms: buried into the shoulders
  const armR = limb(0.09, 0.38, 0.16, 1.12, 0.1);
